...
public class BestIndex {
	...
	static HashSet<String> hasSet = new HashSet<>();
	static HashSet<Integer> hasSetInt = new HashSet<>();
	public static void main(String[] args) throws NumberFormatException, IOException{
		BestIndex bi = new BestIndex();
		int arr[] =  new int[]{5, -2, 7, -3};
		bi.subArray(arr);
		/*String str = "aev";
		
		HashSet<String> setOfVowels = new HashSet<String>();
		for(String sv:bi.findAllSubsequence(str, hasSet)){
			if(!setOfVowels.contains(sv)){
				setOfVowels.add(sv);
			}
		}
		System.out.println(setOfVowels.size());*/
	...
	static public HashSet<String> findAllSubsequence(String str, HashSet<String> hasSet){
		int n = str.length();
		for(int i = 0;i<n;i++){
			for(int j = str.length(); j > i; j--){
				String subStr = str.substring(i, j);
				if(!hasSet.contains(subStr)){
					hasSet.add(subStr);
				}
				for (int k = 1; k < subStr.length() - 1; k++) { 
                    StringBuffer sb = new StringBuffer(subStr); 
                    sb.deleteCharAt(k); 
                    if (!hasSet.contains(sb)){
                    findAllSubsequence(sb.toString(),hasSet); 
                    }
                } 
			}
		}
	...
	public void subArrayOfhalf(){
		int[] arr = new int[]{2, 1, 4, 3, 2, 5, 1, 1, 2, 1};
	}
	...
	public void subArray(int arr[]){
        ...
        { 
            ...
            { 
            	...
            	System.out.println(sumofSubArray(subArr));
            } 
